...
public class VaadinMatcherFilter implements Filter {

  private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(VaadinMatcherFilter.class);
   ...
   *          The route matcher
   * @param staticFiles
   *          The static files configuration object
   * @param externalContainer
   *          Tells the filter that Spark is run in an external web container.
   *          If true, chain.doFilter will be invoked if request is not consumed
   *          by Spark.
   * @param hasOtherHandlers
   *          If true, do nothing if request is not consumed by Spark in order
   *          to let others handlers process the request.
   */
  public VaadinMatcherFilter(spark.route.Routes routeMatcher, StaticFilesConfiguration staticFiles,
  ...
  public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain chain)
    ...
    RouteContext context = RouteContext.create().withMatcher(routeMatcher).withHttpRequest(httpRequest).withUri(uri)
    ...
    try {
      ...
      // If redirected and content is null set to empty string to not throw
      ...
      if (body.notSet() && chain != null && externalContainer) {
